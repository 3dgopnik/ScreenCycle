...
Во время отдыха приложение возвращает пользователя на главный экран и показывает полноэкранное окно «Сейчас отдых». Сообщение и цветовая схема окна можно настроить из главного экрана. Служба цикла сама запускает окно сразу при переходе в фазу REST, поэтому экран отдыха появляется без задержки. Окно работает на Android 8+ без дополнительных условий.
